...
class Sam3Image(torch.nn.Module):
    ...
    def _compute_matching(self, out, targets):
        if targets is None or self.matcher is None:
            return out
    ...
    def back_convert(self, targets):
        if targets is None:
            return None
        batched_targets = {
            "boxes": targets.input_boxes.view(-1, 4) if targets.input_boxes is not None else targets.img_ids.new_empty(0, 4),
            "boxes_xyxy": box_cxcywh_to_xyxy(targets.input_boxes.view(-1, 4)) if targets.input_boxes is not None else targets.img_ids.new_empty(0, 4),
            "boxes_padded": targets.input_boxes if targets.input_boxes is not None else targets.img_ids.new_empty(0, 4),
            "positive_map": targets.input_boxes.new_ones(len(targets.input_boxes), 1) if targets.input_boxes is not None else targets.img_ids.new_empty(0, 1),
            "num_boxes": torch.tensor([len(targets.input_boxes)], device=targets.img_ids.device) if targets.input_boxes is not None else torch.tensor([0], device=targets.img_ids.device),
            "masks": None,
            "semantic_masks": None,
            "is_valid_mask": None,
            "is_exhaustive": None,
            "object_ids_packed": None,
            "object_ids_padded": None,
        }
