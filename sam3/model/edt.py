...
def edt_triton(data: torch.Tensor):
    ...
    if not TRITON_AVAILABLE:
        # Fallback to CPU-based distance transform
        try:
            import scipy.ndimage
            # Convert to numpy, compute EDT, convert back
            data_np = data.cpu().numpy()
            edt_np = scipy.ndimage.distance_transform_edt(data_np == 0)
            return torch.from_numpy(edt_np).to(data.device)
        except ImportError:
            raise ImportError("Triton not available and scipy not found. Please install scipy or triton.")
